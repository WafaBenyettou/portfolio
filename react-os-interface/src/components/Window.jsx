...
function Window({ title, content, onClose }) {
  ...
  return (
    <Draggable handle=".handle">
      <div className={`absolute bg-gray-900 border border-gray-700 shadow-lg rounded-lg ${isMaximized ? 'w-full h-full' : 'min-w-[300px] max-w-[800px]'} ${isMinimized ? 'hidden' : ''} resize overflow-hidden`}>
        ...
        <div className="bg-gray-900 text-white p-4 overflow-auto h-[500px]">{content}</div>
      </div>
    </Draggable>
  );
}
